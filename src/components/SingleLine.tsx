import {
    AcceptedValues, AvailabilityData, DevRow,
} from '../types/types';
...
export const SingleLine = ({
    name, availability, devIndex, setAvailability, onRowClick,
}: TLlineProps) => {
    ...
    const handleClick = (index: number) => {
        ...
        const getNewArray = () => availability.map((row, rowIndex) => {
            if (rowIndex === devIndex) {
                return row.map((value, colIndex) =>
                    (colIndex === index ? getNewValue(value) : value)) as DevRow;
            }
            return row;
        });
